...
function checkform()
{
	...
	var v1 = document.getElementById("priceid").value;
...
<div class="well">
    ...
    <form id="tab"	method="post" action="${url }" onsubmit="return checkform()" enctype="multipart/form-data">
        <label>商品分类名</label>
        <select name="fenlei">
<c:forEach items="${fenleilist}" var="bean22">
<option value="${bean22.id }" <c:if test="${bean.fenlei.id==bean22.id }">selected</c:if> >${bean22.name }</option>
</c:forEach>
</select>
        ...
        <label>销售单价</label>
        <input type="text" name="price" id="priceid" class="input-xlarge" value="${bean.price }">
